...
class ConstantPoolItem:
    ...
    def __init__ (self, text):
        ...
        if self.type == "Class":
            ...
            if (args[3].startswith("Infinity")):
                self.value = "Infinity"
            elif (args[3].startswith ("-Infinity")):
    ...
    def attach (self, parent):
        elmt = ET.SubElement (parent, self.type + "_" + str(self.index))
        attrs = dir (self)
        attrs.remove ('index')
        attrs.remove ('type')
...
class ParseThread (threading.Thread):
    ...
    def run (self):
        print ("Reading", self.in_path, "to", self.out_path)

        result = subprocess.run (
            ["javap", "-verbose", self.in_path],
            stdin = subprocess.DEVNULL, stdout = subprocess.PIPE, stderr = subprocess.DEVNULL).stdout.decode ('utf-8')

        cp = ET.Element ('constant_pool', { 'file': os.path.abspath (self.in_path) } )
...
for root, dirs, files in os.walk (rtdir, followlinks = True):
    for name in files:
        ...
        out_path = os.path.join (outdir, name.replace(".class", ".constant_pool.xml"))
        
        thread = ParseThread (in_path, out_path)
        thread.run()
