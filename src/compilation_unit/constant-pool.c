...
{
    /* Stop if something is wrong with &tmp_error */
    ...
    for (gint i = 1; i < constant_pool_count; i ++)
    {
        ...
        {
            length = g_data_input_stream_read_uint16 (in, NULL, &tmp_error);
            if (tmp_error) goto pool_error;
            ...
            gchar *normal = j2c_modified_utf8_to_standard (utf8_bytes, length, &tmp_error);
            if (tmp_error)
            {
                g_free (utf8_bytes);
                goto pool_error;
