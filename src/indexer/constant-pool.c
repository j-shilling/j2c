...
j2c_constant_pool_class_init (J2cConstantPoolClass *klass)
{
  ...
  g_object_class_install_property (object_class, PROP_SIZE,
				   g_param_spec_ulong (J2C_CONSTANT_POOL_PROP_SIZE,
						       "size",
						       "The size of the constant pool in a class file.",
						       0,
						       G_MAXSIZE,
						       0,
						       G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
...
j2c_constant_pool_set_property (GObject *object, guint property_id, const GValue *value, GParamSpec *pspec)
{
  ...
  switch (property_id)
	...
	break;
      ...
      case PROP_SIZE:
	self->size = (gsize) g_value_get_ulong (value);
	break;
...
j2c_constant_pool_get_property (GObject *object, guint property_id, GValue *value, GParamSpec *pspec)
{
  ...
  switch (property_id)
	...
	break;
      ...
      case PROP_SIZE:
	g_value_set_ulong (value, (gulong) self->size);
	break;
...
J2cConstantPool *
j2c_constant_pool_new (J2cClassFile *file, GError **error)
{
  ...
  if (!file)
    {
      g_set_error (error,
		   ...
		   "Cannot read constant pool from a null J2cClassFile.");
      return NULL;
    }

  /* Get input stream */
  GDataInputStream *in = j2c_indexed_file_read (J2C_INDEXED_FILE (file), error);
  g_return_val_if_fail (NULL != in, NULL);

  /* Skip to the begining of the constant pool */
  g_return_val_if_fail (-1 != g_input_stream_skip (G_INPUT_STREAM (in), 8, NULL, error), NULL);
  ...
  for (gint i = 1; i < constant_pool_count; i ++)
    {
      ...
      constant_pool_size += sizeof (guint8);
      ...
      else if (tag == CONSTANT_Integer)
	  {
	    ...
	    if (*error) goto pool_error;
	    constant_pool_size += sizeof (guint32);
  ...
  ret = g_object_new (J2C_TYPE_CONSTANT_POOL,
		      ...
		      J2C_CONSTANT_POOL_PROP_SIZE, constant_pool_size,
		      NULL);
  goto cleanup;
...
gsize
j2c_constant_pool_size (J2cConstantPool *self)
{
  ...
  g_object_get_property (G_OBJECT (self),
			 J2C_CONSTANT_POOL_PROP_SIZE,
			 &val);

  gsize ret = (gsize) g_value_get_ulong (&val);
  g_value_unset (&val);
